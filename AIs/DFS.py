...
def DFS(maze_graph, initial_vertex) :
    ...
    add_to_explored_vertices(explored_vertices,initial_vertex)#   explored_vertices = [initial_vertex]

    LIFO_push(queuing_structure,initial_vertex) # push the initial vertex to the queuing_structure
    
    while len(queuing_structure) > 0: #   while queuing_structure is not empty:
        current_vertex= queuing_structure.pop()
        ...
        for neighbor,cost in neighbors_costs.items():
            ...
            if not (is_explored(explored_vertices,neighbor)):
                ...
                LIFO_push(queuing_structure,neighbor)
